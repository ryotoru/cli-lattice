...
let originalBasis = Array.from({ length: dimension }, (_, i) => Array(dimension).fill(0).map((_, j) => (i === j ? 1 : 0)));
...
<body>
  ...
  <script>
    ...
    let originalBasis = ${JSON.stringify(originalBasis)};
    ...
    const visualizeDualLattice = () => {
      ...
      try {
        ...
        const originalLatticePoints = generateLatticePoints(basis, ${sumLimit});
        const dualLatticePoints = generateLatticePoints(dualBasis, ${sumLimit});
    ...
    const addParallelepiped = () => {
      ...
      parallelepipedMesh.position.set(
        (point1[0] + point2[0]) / 2,
        (point1[1] + point2[1]) / 2,
        ${dimension} === 2 ? 0 : (point1[2] + point2[2]) / 2
      );
    ...
    const getRandomPoints = () => {
      const points = generateLatticePoints(originalBasis, ${sumLimit});
    ...
    const initialLatticePoints = generateLatticePoints(originalBasis, ${sumLimit});
